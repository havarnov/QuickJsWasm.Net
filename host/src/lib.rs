...
pub extern "C" fn init(wasm: *const u8, length: usize) -> *mut RuntimeContextResult {
    unsafe {
        ...
        let engine = match Engine::new(&config) {
            Ok(engine) => engine,
            Err(error) => {
                return RuntimeContextResult::error(
                    RuntimeContextErrorTag::Engine,
                    &error.to_string(),
                );
            }
        };
        ...
        let component = match Component::from_binary(&engine, wasm) {
            ...
            Err(err) => {
                return RuntimeContextResult::error(
                    RuntimeContextErrorTag::Component,
                    &err.to_string(),
                );
            }
        };
        ...
        let engine_instance = match api.engine().call_create(&mut store) {
...
pub struct EvalResult {
    eval_result: *mut Param,
    error: *mut EvalError,
    is_ok: bool,
}

#[unsafe(no_mangle)]
pub extern "C" fn eval(ctx: *mut RuntimeContext, script: *const c_char) -> *mut EvalResult {
    unsafe {
            ...
            .call_eval(&mut ctx.store, ctx.instance, &script_str)
        {
            Ok(Ok(result)) => {
                let result = match result {
                    callback_api::Param::Unit => Box::new(Param::new_unit()),
                    callback_api::Param::Int(None) => Box::new(Param::new_null()),
                    callback_api::Param::Int(Some(i)) => Box::new(Param::new_int(i)),
                    _ => todo!(),
                };
                let result = Box::into_raw(result) as *mut Param;
                Box::new(EvalResult {
                    eval_result: result,
                    error: std::ptr::null_mut(),
                    is_ok: true,
        ...
        Box::into_raw(result) as *mut EvalResult
    }
}

pub struct Callback {
    func: Arc<
        dyn Fn(&mut ComponentRunStates, String, Vec<callback_api::Param>) -> callback_api::Param
            + Send
            + Sync
            + 'static,
    >,
...
impl crate::callback_api::HostLazyParam for ComponentRunStates {
    ...
    fn get(
        &mut self,
        resource: wasmtime::component::Resource<LazyParam>,
    ) -> Result<callback_api::Param, wasmtime::Error> {
...
impl Param {
    fn new_unit() -> Param {
        Param {
            ...
            list_value: ParamList::new_null(),
        }
    }

    fn new_int(value: i32) -> Param {
        Param {
            tag: ParamTag::Int,
            int_value: value,
            string_value: std::ptr::null_mut(),
...
pub extern "C" fn register(
    ...
    unsafe {
        ...
        let callback = Callback {
            func: Arc::new(
                move |component: _, name: String, params: Vec<callback_api::Param>| {
                    ...
                    let params: Vec<Param> = params
                        .into_iter()
                        .map(|p| match p {
                            callback_api::Param::Unit => Param {
                                tag: ParamTag::Unit,
                                int_value: 0,
                                string_value: std::ptr::null_mut(),
                                list_value: ParamList::new_null(),
                            },
                            callback_api::Param::Int(value) => Param {
                                tag: ParamTag::Int,
                                int_value: value.unwrap_or(0),
                                string_value: std::ptr::null_mut(),
                                list_value: ParamList::new_null(),
                            },
                            callback_api::Param::Str(None) => Param {
                                tag: ParamTag::String,
                                int_value: 0,
                                string_value: std::ptr::null_mut(),
                                list_value: ParamList::new_null(),
                            ...
                            callback_api::Param::Null => Param {
                                tag: ParamTag::Null,
                                int_value: 0,
                                string_value: std::ptr::null_mut(),
                                list_value: ParamList::new_null(),
                            },
                            callback_api::Param::Vec(None) => Param {
                                tag: ParamTag::List,
                                int_value: 0,
                                string_value: std::ptr::null_mut(),
                                list_value: ParamList::new_null(),
                            ...
                            callback_api::Param::Vec(Some(value)) => {
                                ...
                                for inner in value {
                                    let inner =
                                        crate::callback_api::HostLazyParam::get(component, inner)
                                            .unwrap();
